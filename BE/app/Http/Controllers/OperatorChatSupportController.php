...
/**
 * Nhà xe — yêu cầu hỗ trợ BusSafe qua {@see LiveSupportSession} (target admin, thread_type nha_xe).
 */
final class OperatorChatSupportController extends Controller
{
    public function index(Request $request, LiveSupportBusSafeNhaXeStaffService $svc): JsonResponse
    {
        ...
        $sessions = $svc->paginateForOperator($request, $nhaXe);
    ...
    public function statsDaily(Request $request, ChatSupportDailyStatsService $stats): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'data' => [
                'daily' => $stats->dailySeriesLiveSupportNhaXeBusafe($ma, $from, $to),
            ],
        ]);
    }
    ...
    public function show(Request $request, int $id, LiveSupportBusSafeNhaXeStaffService $svc): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'data' => $svc->showForOperator($request, $id, $nhaXe),
        ]);
    }
    ...
    public function store(Request $request, LiveSupportBusSafeNhaXeStaffService $svc): JsonResponse
    {
        ...
        $session = $svc->createFromOperator($nhaXe, $data['tieu_de'], $data['noi_dung']);
