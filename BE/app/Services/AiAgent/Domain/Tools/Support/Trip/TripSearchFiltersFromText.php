...
final class TripSearchFiltersFromText
{
    ...
    public static function build(array $arguments, array $state = []): array
    {
        ...
        if (isset($arguments['ngay_di']) || isset($state['entities']['ngay_di'])) {
            $ngay = $arguments['ngay_di'] ?? $state['entities']['ngay_di'];
        }
        ...
        // Ưu tiên lấy từ entities do AI bóc tách
        if (isset($state['entities']['diem_di'])) {
            $filters['diem_di'] = $state['entities']['diem_di'];
        }
        if (isset($state['entities']['diem_den'])) {
            $filters['diem_den'] = $state['entities']['diem_den'];
        }

        if (isset($filters['diem_di']) && isset($filters['diem_den'])) {
            // Đã đủ thông tin từ AI, bỏ qua regex
        } else {
            ...
            } elseif ($msg !== '' && preg_match('/(?:từ|ở|tại)\s+(.+?)\s+(?:đến|tới|về|ra|vào)\s+(.+)/u', $msg, $mT)) {
            ...
            } elseif ($msg !== '' && preg_match('/(?:đến|tới|về|ra|vào)\s+(.+)/u', $msg, $m4)) {
                $filters['diem_den'] = trim($m4[1]);
            } elseif ($msg !== '' && preg_match('/(?:từ|ở|tại|đi)\s+(.+)/u', $msg, $m5)) {
                $filters['diem_di'] = trim($m5[1]);
            } elseif ($msg !== '') {
                // Thử lấy từ province_matches nếu có
                $matches = $state['normalized']['province_matches'] ?? [];
                if (count($matches) >= 2) {
                    $filters['diem_di'] = $matches[0]['ten_tinh_thanh'] ?? $filters['diem_di'] ?? null;
                    $filters['diem_den'] = $matches[1]['ten_tinh_thanh'] ?? $filters['diem_den'] ?? null;
                } elseif (count($matches) === 1) {
                    $filters['diem_di'] = $matches[0]['ten_tinh_thanh'] ?? $filters['diem_di'] ?? null;
                } else {
                    $filters['diem_di'] = $filters['diem_di'] ?? $msg;
                }
            }
        }
    ...
    {
        $s = trim($s);
        // Loại bỏ các tiền tố hành động và từ nối phổ biến
        $s = preg_replace('/^(đặt\s+vé|mua\s+vé|tìm\s+chuyến|tra\s+cứu|book|xem|lấy|giữ|xe|chuyến)\s*[:\-]?\s*/iu', '', $s) ?? $s;
        $s = preg_replace('/^(đi|từ|đến|tới|về|ra|vào|ở|tại)\s+/iu', '', $s) ?? $s;
