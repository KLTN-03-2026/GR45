...
const toggleStatus = async (id) => {
  if (confirm('Bạn có chắc muốn chuyển đổi trạng thái tài khoản này không?')) {
    try {
...
const requestDelete = async (id) => {
  if (confirm('LƯU Ý: Đây là hành động xoá VĨNH VIỄN khỏi cơ sở dữ liệu. Bạn có chắc chắn?')) {
    try {
...
<template>
  <div class="admin-page">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">Quản Lý Toàn Bộ Tài Xế</h1>
        <p class="page-sub">Administrator view: Xem, thêm, sửa, xóa, khóa hoạt động toàn bộ tài xế hệ thống.</p>
      </div>
      ...
      <BaseTable :columns="tableColumns" :data="drivers" :loading="loading">
        ...
        <template #cell(thong_tin)="{ item }">
          <div class="info-block">
            <strong>{{ item.hoSo?.ho_va_ten || 'Thiếu tên' }}</strong>
            <span class="sub-text">{{ item.email }}</span>
          </div>
        </template>

        <template #cell(nha_xe)="{ item }">
           <span class="code-chip">{{ item.ma_nha_xe || 'Trống' }}</span>
           <div class="sub-text" style="margin-top: 2px;">
               {{ item.nhaXe?.ten_nha_xe || '' }}
           </div>
        </template>
        ...
        <template #cell(actions)="{ item }">
          <div class="action-buttons">
            ...
            <BaseButton size="sm" variant="secondary" @click="toggleStatus(item.id)">
               {{ item.tinh_trang === 'hoat_dong' ? 'Bấm khoá' : 'Mở khoá' }}
            </BaseButton>
    ...
    >
      ...
      <form @submit.prevent="submitForm" class="driver-form">
        ...
        <div class="file-grid">
          ...
          <div class="file-group">
            <label class="base-input-label">GPLX Trước <span v-if="!isEditMode">*</span></label>
          ...
          <div class="file-group">
            <label class="base-input-label">GPLX Sau <span v-if="!isEditMode">*</span></label>
      ...
      <template #footer>
        ...
        <BaseButton variant="primary" :loading="formLoading" @click="submitForm">
          {{ isEditMode ? 'Ép Lưu Quyền Admin' : 'Thêm tài xế' }}
        </BaseButton>
      </template>
    </BaseModal>
