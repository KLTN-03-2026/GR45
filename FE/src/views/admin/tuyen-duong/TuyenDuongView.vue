...
const addStation = () => {
  formData.tram_dungs.push(
    getEmptyStation("don", formData.tram_dungs.length + 1),
  );
};
...
const fetchStationCoordinates = async (station, index) => {
  ...
  try {
    ...
    // mapApi tự rate-limit (≥1.2s/req) + retry 429 — không cần delay thủ công
    for (const query of queries) {
      const response = await mapApi.searchCoordinatesByAddress(query);
      if (response?.data?.length) {
        result = response.data[0];
        break; // Dừng ngay khi có kết quả
      }
    }
    ...
    showToast(
      `✓ Tọa độ: ${Number(result.lat).toFixed(5)}, ${Number(result.lon).toFixed(5)}`,
      "success",
    );
  } catch (error) {
...
const submitForm = async () => {
  try {
    ...
    showToast(
      error.response?.data?.message || "Có lỗi xảy ra khi lưu tuyến đường!",
      "error",
    );
  } finally {
...
const openConfirmModal = (action, id) => {
  ...
  if (action === "approve") {
    ...
    confirmModal.message =
      "Bạn có chắc muốn xóa tuyến đường này không? Hành động này không thể hoàn tác!";
  }
...
const executeConfirmAction = async () => {
  ...
  try {
    ...
    showToast(
      error.response?.data?.message || `Có lỗi khi thực hiện ${action}!`,
      "error",
    );
  } finally {
...
<template>
  <div class="admin-page">
    ...
    <div class="table-card">
      <BaseTable :columns="tableColumns" :data="routes" :loading="loading">
        ...
        <template #cell(actions)="{ item }">
          <div class="action-buttons">
            ...
            <BaseButton
              size="sm"
              variant="danger"
              @click="openConfirmModal('delete', item.id)"
              >Xóa</BaseButton
            >
          </div>
        </template>
    ...
    >
      <form @submit.prevent="submitForm" class="route-form">
        ...
        <div class="form-grid">
          <BaseInput
            v-model="formData.ma_nha_xe"
            label="Mã Nhà Xe"
            placeholder="Nhập mã nhà xe (VD: NX001)"
            required
          />
          <BaseInput
          ...
          <div class="form-group full-width" v-if="isEditMode">
            ...
            <div
              ...
              role="group"
              aria-label="Trạng thái tuyến đường"
            >
              <input
                type="radio"
                class="btn-check"
                ...
                autocomplete="off"
                value="hoat_dong"
                v-model="formData.tinh_trang"
              />
              <label class="btn btn-outline-success" for="statusActive"
                >Đã Phê Duyệt / Hoạt Động</label
              >
          ...
          >
            ...
            <div class="form-grid">
              <BaseInput
                ...
                label="Tên Trạm"
                placeholder="VD: Bến xe Nước Ngầm"
                required
              />
              <div class="form-group">
