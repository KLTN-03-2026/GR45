...
const fetchTicketsAnalytics = async (tuNgay, denNgay, gen) => {
  ...
  totalStats.value.tongVeDaBan = acc.filter((t) => isDaThanhToan(t)).length
...
/** Tổng doanh thu KPI = tổng tiền các thanh toán hợp lệ trong kỳ (toàn bộ trang, cùng tham số ngày với bảng). */
const applyPaymentRevenueTotal = async (tuNgay, denNgay, gen) => {
  ...
  try {
    ...
    for (;;) {
      ...
      lastPage = Math.max(1, Number(payload?.last_page ?? 1) || 1)
      for (const pm of rows) {
        if (isPmThanhToanCong(pm)) sum += getPmSoTienForTotal(pm)
      }
...
const handleApplyFilter = async () => {
  ...
  const tasks = [fetchPayments()]
  if (!routeTop.value.length || pie === 0) tasks.push(fetchTicketsAnalytics(tuNgay, denNgay, myGen))
...
onMounted(async () => {
  ...
  const pie =
    ...
    ticketVeSlice.value.tienMat +
    ticketVeSlice.value.daHuy
  const tasks = [fetchPayments(), fetchKhachHang(), fetchNhaXe(), fetchChuyenXe()]
  if (!routeTop.value.length || pie === 0) tasks.push(fetchTicketsAnalytics(tuNgay, denNgay, myGen))
  if (!totalStats.value.khachHangMoi) tasks.push(fetchNewClientsInRange(tuNgay, denNgay))
  await Promise.all(tasks)
})
