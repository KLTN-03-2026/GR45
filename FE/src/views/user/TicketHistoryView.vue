...
// inject dùng để nhận dữ liệu từ component cha
...
async function loadAllTickets() {
  ...
  try {
    ...
    listMessage.value = {
      type: "error",
      text: "Không thể tải lịch sử đặt vé.",
    };
    allTickets.value = [];
  } finally {
...
async function openDetail(ticket) {
  ...
  try {
    ...
    detailTicket.value = ticket;
    detailError.value =
      "Không tải được chi tiết đầy đủ từ máy chủ — hiển thị theo dữ liệu danh sách.";
  } finally {
...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900 font-sans py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        <div>
          ...
          <p class="text-slate-500 text-sm mt-1">
            Danh sách vé theo thời gian mới nhất.
          </p>
        </div>
      </div>

      <div
        v-if="listMessage.text"
        :class="
          listMessage.type === 'error'
            ? 'bg-red-50 text-red-700 border-red-200'
            : 'bg-amber-50 text-amber-800 border-amber-200'
        "
        class="mb-6 p-4 rounded-2xl text-sm font-medium border"
      >
        {{ listMessage.text }}
      </div>
      ...
      <!-- Lọc trạng thái -->
      <div
        class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 mb-6"
      >
        <p
      ...
      >
            ...
            >
              <div class="min-w-0 flex-1 space-y-3">
                <div class="flex flex-wrap items-center gap-2">
                  <span
                    :class="statusBadgeClass(ticket)"
                    class="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide border"
                  >
                    {{ statusLabel(ticket) }}
                  </span>
                  <span class="text-slate-400 text-xs font-mono"
                    >Mã vé: {{ ticket.ma_ve }}</span
                  >
                </div>
                <div class="flex items-start gap-3 text-slate-900">
                  <span class="material-symbols-outlined text-blue-600 shrink-0"
                    >route</span
                  >
                  <div>
                    ...
                    <p
                      v-if="ticket.chuyen_xe?.tuyen_duong?.ten_tuyen_duong"
              ...
              <div class="text-right shrink-0">
                ...
                <button
                  v-if="
                    ticket.tinh_trang === 'da_hoan_thanh' ||
                    categoryOf(ticket) === 'hoan_thanh'
                  "
                  type="button"
                  @click.stop="rateTrip(ticket)"
                  class="text-xs text-blue-600 font-semibold mt-2 hover:underline inline-block w-full text-right"
                >
                  Đánh giá chuyến xe →
                </button>
              </div>
        ...
        <!-- Phân trang (client) -->
        <div
          v-if="!isLoading && filteredTickets.length > pageSize"
          class="flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-4 bg-slate-50/80 border-t border-slate-100"
        >
          <p class="text-sm text-slate-500">
            ...
            <span class="text-slate-400"
              >({{ filteredTickets.length }} vé)</span
            >
          </p>
        ...
        >
          ...
          >
            ...
            <section class="space-y-2">
              ...
              <p
                v-if="detailTicket.chuyen_xe?.tuyen_duong?.ten_tuyen_duong"
                class="text-sm text-slate-600"
              >
                {{ detailTicket.chuyen_xe.tuyen_duong.ten_tuyen_duong }}
              </p>
              <p
                v-if="detailTicket.chuyen_xe?.tuyen_duong?.nha_xe?.ten_nha_xe"
                class="text-sm text-slate-500"
              >
                Nhà xe:
                {{ detailTicket.chuyen_xe.tuyen_duong.nha_xe.ten_nha_xe }}
              </p>
              <div class="flex flex-wrap gap-4 text-sm pt-2">
                <span
                  >Ngày khởi hành:
                  <strong>{{
                    formatDate(detailTicket.chuyen_xe?.ngay_khoi_hanh)
                  }}</strong></span
                >
                <span
                  >Giờ:
                  <strong>{{
                    formatTime(detailTicket.chuyen_xe?.gio_khoi_hanh)
                  }}</strong></span
                >
              </div>
            </section>
            ...
            <section class="space-y-2">
              ...
              <p class="text-sm">
                Người đặt:
                <strong>{{ detailTicket.khach_hang?.ho_va_ten || "—" }}</strong>
                <span
                  v-if="detailTicket.khach_hang?.so_dien_thoai"
                  class="text-slate-600"
                >
                  — {{ detailTicket.khach_hang.so_dien_thoai }}</span
                >
              </p>
              <p class="text-sm">
                Phương thức:
                {{ paymentLabel(detailTicket.phuong_thuc_thanh_toan) }}
              </p>
              <p class="text-sm">
                Thời gian đặt: {{ formatDateTime(detailTicket.thoi_gian_dat) }}
              </p>
              <p v-if="detailTicket.thoi_gian_thanh_toan" class="text-sm">
                Thanh toán lúc:
                {{ formatDateTime(detailTicket.thoi_gian_thanh_toan) }}
              </p>
              <div
                class="flex flex-wrap gap-6 pt-2 border-t border-slate-100 mt-2"
              >
                <div>
                  <p class="text-xs text-slate-500">Tổng tiền</p>
                  <p class="text-xl font-bold text-blue-700">
                    {{ formatCurrency(detailTicket.tong_tien) }}
                  </p>
                </div>
                <div v-if="detailTicket.tien_khuyen_mai > 0">
                  ...
                  <p class="text-lg font-semibold text-emerald-700">
                    -{{ formatCurrency(detailTicket.tien_khuyen_mai) }}
                  </p>
                </div>
              </div>
            </section>
              ...
              <ul class="space-y-2">
                ...
                >
                  <span class="font-bold"
                    >Ghế {{ ct.ghe?.ma_ghe || ct.ma_ghe || "—" }}</span
                  >
                  <span v-if="ct.tram_don?.ten_tram" class="text-slate-600">
                    — Đón: {{ ct.tram_don.ten_tram }}</span
                  >
                  <span v-if="ct.tram_tra?.ten_tram" class="text-slate-600">
                    — Trả: {{ ct.tram_tra.ten_tram }}</span
                  >
                  <span
                    v-if="ct.gia_ve != null"
                    class="block text-slate-500 mt-1"
                    >{{ formatCurrency(ct.gia_ve) }}</span
                  >
                </li>
              </ul>
            </section>
