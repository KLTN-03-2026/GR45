...
const seatsFloor1 = computed(() =>
  seatMapRaw.value.filter((s) => s.tang === 1),
);
const seatsFloor2 = computed(() =>
  seatMapRaw.value.filter((s) => s.tang === 2),
);
...
const fetchInitialData = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      // Load vouchers
      const vouchersRes = await clientApi.getPublicVouchers({
      ...
      <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="lg:w-2/3">
          ...
          >
            ...
            <!-- Chú thích -->
            <div
              class="flex justify-center gap-6 mb-8 text-sm text-slate-600 flex-wrap"
            >
              <div class="flex items-center gap-2">
                <div
                  class="w-5 h-5 bg-white border-2 border-slate-200 rounded"
                ></div>
                Ghế trống
              </div>
              <div class="flex items-center gap-2">
                <div
                  class="w-5 h-5 bg-slate-200 border-2 border-slate-300 rounded opacity-60"
                ></div>
                Đã đặt
              </div>
              <div class="flex items-center gap-2">
                <div
                  class="w-5 h-5 bg-blue-100 border-2 border-blue-500 rounded"
                ></div>
                Đang chọn
              </div>
            </div>
            ...
            <div
              class="seat-map-container bg-slate-50 p-6 rounded-2xl border border-slate-100 w-fit mx-auto"
            >
              <!-- Đầu xe -->
              <div
                class="w-full text-center text-xs font-bold text-slate-400 mb-6 uppercase tracking-widest border-b-2 border-dashed border-slate-200 pb-2"
              >
                Đầu xe
              </div>
              ...
              <div class="grid grid-cols-5 gap-x-2 sm:gap-x-4 gap-y-4">
                <template
                  v-for="(seat, index) in currentFloor === 1
                    ? seatsFloor1
                    : seatsFloor2"
                  :key="seat.id_ghe"
                >
                  ...
                  <button
                    class="seat-btn relative w-10 sm:w-12 h-14 sm:h-16 rounded-lg border-2 transition-all flex flex-col items-center justify-center group"
                    :disabled="seat.trang_thai !== 'trong'"
                    :class="[
                      seat.trang_thai !== 'trong'
                        ? 'bg-slate-200 border-slate-300 cursor-not-allowed opacity-60'
                        : isSeatSelected(seat.id_ghe)
                  ...
                  >
                    ...
                    <span
                      v-if="isSeatSelected(seat.id_ghe)"
                      class="material-symbols-outlined text-blue-500 text-[16px] mt-1 absolute bottom-1"
                      >check_circle</span
                    >
                  </button>
                </template>
          ...
          >
            ...
            <div class="grid md:grid-cols-2 gap-8">
              <!-- Điểm đón -->
              <div>
                <h3 class="font-semibold text-slate-700 mb-4 pb-2 border-b">
                  Điểm Đón
                </h3>
                <div
                ...
                <div
                  class="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scroll"
                >
                  <label
                  ...
                  >
                    ...
                    <div class="card-content">
                      <span
                        class="font-semibold block text-slate-800 text-sm mb-1"
                        >{{ stop.ten_tram }}</span
                      >
                      <span class="text-xs text-slate-500 flex gap-1"
          ...
          >
            ...
            <!-- Khuyến mãi -->
            <div
              class="mb-8 p-5 bg-blue-50/50 rounded-2xl border border-blue-100"
            >
              <h3
              ...
              <div v-if="vouchers.length === 0" class="text-slate-500 text-sm">
                Không có mã giảm giá nào phù hợp.
              </div>
              <div
                v-else
                class="grid sm:grid-cols-2 gap-3 max-h-[200px] overflow-y-auto pr-2 custom-scroll"
              >
                <label v-for="vc in vouchers" :key="vc.id" class="voucher-card">
                  ...
                  />
                  <div
                    class="card-content flex items-center p-3 border rounded-xl cursor-pointer transition-all hover:bg-white bg-slate-50"
                  >
                    <div
                      class="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold mr-3 shrink-0"
                    >
                      %
                    </div>
                    <div class="flex-1 min-w-0">
                      <div
                        class="font-bold text-slate-800 text-sm mb-0.5 truncate"
                      >
                        {{ vc.ma_voucher }}
                      </div>
              ...
              <div
                ...
                class="mt-3 text-sm text-blue-600 font-medium cursor-pointer flex justify-end"
                @click="selectedVoucherId = null"
              >
                Bỏ chọn mã
              </div>
            </div>
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <div class="card-content flex items-center">
                    <span
                      class="material-symbols-outlined text-emerald-500 mr-3 text-2xl"
                      >payments</span
                    >
                    <div class="flex-1">
                ...
                <div
                  v-else
                  class="p-3 bg-amber-50 text-amber-700 rounded-xl text-sm flex items-center gap-2 mb-3 border border-amber-200"
                >
                  <span
                ...
                <label class="radio-card block">
                  ...
                  <div class="card-content flex items-center">
                    <span
                      class="material-symbols-outlined text-blue-500 mr-3 text-2xl"
            ...
            <div class="flex justify-between mt-8 border-t pt-6">
              ...
              <button
                ...
                :disabled="isBooking"
                class="btn-primary"
                :class="{ 'opacity-75 cursor-wait': isBooking }"
              >
                <span
                  v-if="isBooking"
          ...
          >
            ...
            >
              <span
                v-if="bookingResult.tinh_trang === 'da_thanh_toan'"
            ...
            <h2 class="text-2xl font-bold text-slate-800 mb-2">
              <template v-if="bookingResult.tinh_trang === 'da_thanh_toan'"
        ...
        <div class="lg:w-1/3">
          <div class="glass-card rounded-2xl p-5 sticky top-24">
            <h3 class="font-bold text-lg mb-4 text-slate-800 pb-3 border-b">
              Thông tin đặt vé
            </h3>
            ...
            <div class="space-y-4 mb-6">
              <div>
                <span
                  class="block text-xs text-slate-400 font-medium uppercase tracking-wider mb-1"
                  >Chuyến xe</span
                >
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2 mt-1">
                  <span
                    ...
                    :key="s.id_ghe"
                    class="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-1 rounded inline-block"
                  >
                    {{ s.ma_ghe }}
                  </span>
                  <span
