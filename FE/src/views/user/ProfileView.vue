...
const openDetailedRating = inject("openDetailedRating", null);

const rateTrip = (ticket) => {
  if (!openDetailedRating) return;
  const cx = ticket.chuyen_xe || {};
  const td = cx.tuyen_duong || {};
  const nx = td.nha_xe || {};
  const xe = cx.xe || {};

  const maVeList = ticket.chi_tiet_ves?.length
    ? ticket.chi_tiet_ves.map((ct) => ct.ma_ve || ct.ghe?.ma_ghe)
    : [ticket.ma_ve];

  openDetailedRating({
    trip_id: cx.id,
    ma_ve_list: maVeList,
    ticket_count: ticket.so_luong || 1,
    ...
    gio_khoi_hanh: cx.gio_khoi_hanh ? cx.gio_khoi_hanh.substring(0, 5) : null,
  });
};
...
const userEmail = computed(
  () => profileForm.email || clientStore.user?.email || "",
);

const anyProfileModalOpen = computed(
  () => showMyRatingDetail.value || showTicketModal.value,
);
...
const getRatingRouteLine = (rating) => {
  ...
  const a =
    from != null && String(from).trim() !== "" ? String(from).trim() : "—";
...
const fetchTickets = async () => {
  ...
  try {
    ...
    ticketsMessage.value = {
      type: "error",
      text: "Không thể tải danh sách vé.",
    };
    allTickets.value = [];
  } finally {
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside class="w-full lg:w-72 flex-shrink-0 lg:sticky lg:top-8 h-fit">
          ...
          >
            ...
            <nav class="p-4 flex flex-col gap-1">
              <div class="mb-1">
                <button
                  @click="expandedMenus.account = !expandedMenus.account"
                ...
                >
                  ...
                  <span
                    class="material-symbols-outlined text-slate-400 text-base"
                  >
                ...
                <div v-show="expandedMenus.account" class="ml-4 mt-1 space-y-1">
                  <button
                    @click="selectMenu('account', 'profile')"
                    :class="[
                      'w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all',
                      activeMenu.main === 'account' &&
                      activeMenu.sub === 'profile'
                  ...
                  <button
                    @click="selectMenu('account', 'security')"
                    :class="[
                      'w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all',
                      activeMenu.main === 'account' &&
                      activeMenu.sub === 'security'
                        ? 'bg-blue-50 text-blue-700 font-semibold'
                        : 'text-slate-600 hover:bg-slate-50',
                    ]"
                  >
              ...
              <button
                @click="(selectMenu('my-ratings', null), fetchRatingsData())"
                :class="[
                  'flex items-center gap-3 px-4 py-3 rounded-2xl font-semibold transition-all duration-200',
                  activeMenu.main === 'my-ratings'
                    ? 'bg-blue-50 text-blue-700'
                    : 'text-slate-600 hover:bg-slate-50',
                ]"
              >
              ...
              <div class="mt-1">
                <button
                  @click="expandedMenus.offers = !expandedMenus.offers"
                  class="w-full flex items-center justify-between px-4 py-3 rounded-2xl text-slate-700 hover:bg-slate-50 transition-all duration-200 font-semibold"
                >
                  <div class="flex items-center gap-3">
                    <span class="material-symbols-outlined text-slate-500"
                ...
                <div v-show="expandedMenus.offers" class="ml-4 mt-1 space-y-1">
                  <button
                    @click="selectMenu('offers', 'points')"
                    :class="[
                      'w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all',
                      activeMenu.main === 'offers' &&
                      activeMenu.sub === 'points'
                        ? 'bg-blue-50 text-blue-700 font-semibold'
                        : 'text-slate-600 hover:bg-slate-50',
                    ]"
                  >
                  ...
                  <button
                    @click="selectMenu('offers', 'vouchers')"
                    :class="[
                      'w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all',
                      activeMenu.main === 'offers' &&
                      activeMenu.sub === 'vouchers'
                        ? 'bg-blue-50 text-blue-700 font-semibold'
                        : 'text-slate-600 hover:bg-slate-50',
                    ]"
                  >
        ...
        <main class="flex-1 min-w-0 space-y-8">
          <!-- Header Profile Card -->
          <section
            class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden relative"
          >
            <div
              class="h-32 bg-gradient-to-r from-blue-600 to-indigo-600"
            ></div>
              ...
              <div class="relative flex justify-between items-end -mt-12 mb-6">
                ...
                <div class="mb-2">
                  <span
                    ...
                    class="px-6 py-2 font-bold text-sm rounded-full border shadow-sm"
                    >{{ displayRank.label }}</span
                  >
                </div>
              </div>
              <div>
                ...
                <div
                  class="flex flex-wrap items-center gap-4 mt-3 text-slate-500 text-sm font-medium"
                >
                  ...
                  <span
                    v-if="profileForm.so_dien_thoai"
          ...
          <!-- Profile Content -->
          <div
            v-if="activeMenu.main === 'account' && activeMenu.sub === 'profile'"
            class="space-y-8 animate-fade-in"
          >
            <section
              class="bg-white p-6 sm:p-10 rounded-3xl shadow-sm border border-slate-200"
            >
              <div class="mb-8">
                <h2 class="text-xl font-bold text-slate-900">Hồ sơ cá nhân</h2>
                <p class="text-sm text-slate-500 mt-1">
                  Quản lý thông tin liên hệ và tùy chỉnh tài khoản.
                </p>
              </div>
              <form @submit.prevent="handleUpdateProfile" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="space-y-2">
                    <label
                      class="text-sm font-semibold text-slate-700 flex items-center h-6"
                      >Họ và tên <span class="text-red-500 ml-1">*</span></label
                    >
                    <input
                      type="text"
                      v-model="profileForm.ho_va_ten"
                      required
                      class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 transition-colors h-[50px]"
                    />
                  </div>
                  <div class="space-y-2">
                    <label
                  ...
                  <div class="space-y-2">
                    <label
                      class="text-sm font-semibold text-slate-700 flex items-center justify-between h-6"
                    >
                      <span>Email</span>
                      <span
                    ...
                    <input
                      ...
                      disabled
                      class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 cursor-not-allowed h-[50px]"
                    />
                  </div>
                  <div class="space-y-2">
                    <label
          ...
          <div
            v-if="
              activeMenu.main === 'account' && activeMenu.sub === 'security'
            "
            class="space-y-8 animate-fade-in"
          >
            <section
            ...
            >
              <div class="mb-8">
                ...
                <p class="text-sm text-slate-500 mt-1">
                  Cập nhật mật khẩu để bảo vệ dữ liệu của bạn.
                </p>
              </div>
              <form
          ...
          >
            ...
            <div v-if="isLoadingTickets" class="py-20 text-center">
              <span
                class="material-symbols-outlined animate-spin text-4xl text-blue-600"
                >refresh</span
              >
            ...
            >
              <span class="material-symbols-outlined text-6xl text-slate-200"
                >confirmation_number</span
              >
              <p class="mt-4 text-slate-400 font-bold">
            ...
            <div v-else class="space-y-6">
              ...
              >
                ...
                <div
                  class="absolute top-0 right-0 w-24 h-24 bg-blue-50/50 rounded-bl-full -mr-12 -mt-12 transition-all group-hover:bg-blue-100/50"
                ></div>
                ...
                <div class="flex-1 w-full relative z-10">
                  <div @click="openTicketDetail(ticket)">
                    <div class="flex justify-between items-center mb-8">
                      ...
                      >
                        <span
                          class="material-symbols-outlined text-xs text-slate-400"
                          >confirmation_number</span
                        >
                        <span
                          class="text-slate-900 text-sm font-mono font-bold"
                          >{{ ticket.ma_ve }}</span
                        >
                      </div>
                    </div>
                    ...
                    <div class="flex items-center gap-8 text-slate-900">
                      ...
                      <div class="flex-1 flex flex-col items-center px-4">
                        <div
                          class="text-sm font-bold text-slate-500 mb-2 tracking-widest uppercase"
                        >
                          {{ formatDate(ticket.chuyen_xe?.ngay_khoi_hanh) }}
                        </div>
                        <div
                          class="h-[3px] bg-slate-200 w-full rounded-full"
                        ></div>
                        <div
                          class="mt-3 text-sm font-bold text-slate-500 uppercase tracking-widest text-center leading-tight"
                        >
                    ...
                    <div
                      class="mt-8 pt-6 border-t border-slate-50 flex justify-between items-center"
                    >
                      <div class="flex items-start gap-6">
                        <!-- Giờ khởi hành -->
                        <div class="flex flex-col gap-1">
                          <p
                            class="text-[10px] text-slate-400 font-bold uppercase tracking-widest h-4 flex items-center"
                          >
                            Giờ khởi hành
                          </p>
                          <p
                            class="text-4xl font-bold text-blue-600 tracking-tighter leading-none"
                          >
                            {{
                              ticket.chuyen_xe?.gio_khoi_hanh?.substring(0, 5)
                            }}
                          </p>
                        </div>
                        ...
                        <!-- Đường kẻ dọc -->
                        <div
                          class="h-12 w-px bg-slate-200 mx-2 self-center"
                        ></div>
                        ...
                        <div class="flex flex-col gap-1">
                          ...
                          <p
                            class="text-sm font-bold text-slate-700 leading-none h-[40px] flex items-center"
                          >
                            {{
                              ticket.chuyen_xe?.tuyen_duong?.nha_xe
                                ?.ten_nha_xe || "BusSafe Official"
                            }}
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        {{ formatCurrency(ticket.tong_tien) }}
                      </div>
                    </div>
                  </div>
                ...
                >
                  <div
                    class="bg-slate-50 p-4 rounded-[24px] border border-slate-100 group-hover:bg-blue-50 transition-colors"
                  >
                    <img
              ...
              >
                <div class="text-sm font-bold text-slate-400">
                  Trang <span class="text-slate-900">{{ currentPage }}</span> /
                  {{ totalPages }}
                </div>
          ...
          <!-- My Ratings Content -->
          <div
            v-if="activeMenu.main === 'my-ratings'"
            ...
            <div v-else class="mr-list-wrap">
              ...
              >
                <div class="mr-list-row-main">
                  ...
                  <div class="mr-list-meta">
                    <span
                      ...
                      }}<template v-if="rating.chuyen_gio_khoi_hanh">
                        · {{ rating.chuyen_gio_khoi_hanh }}</template
                      ></span
                    >
                    <span v-else class="mr-list-date">{{
                ...
                <div class="mr-list-row-score">
                  ...
                  <span class="mr-score-num"
                    >{{ clampRatingScore(rating.diem_so) }}/5</span
                  >
                  <span class="material-symbols-outlined mr-list-chevron"
                    >chevron_right</span
                  >
                </div>
              </button>
              ...
              >
                ...
                <section>
                  <div class="flex justify-between items-center mb-6">
                    ...
                    <span
                      :class="getStatusInfoExtended(selectedTicket).class"
                  ...
                  <div class="space-y-4">
                    <div class="flex justify-between items-center">
                      <span class="text-slate-500 font-semibold text-base"
                        >Mã vé</span
                      >
                      <span
                        class="font-bold text-slate-900 font-mono text-xl"
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      <span class="font-bold text-slate-900 text-base"
                        >{{
                          formatDate(selectedTicket.chuyen_xe?.ngay_khoi_hanh)
                        }}
                        ·
                        {{
                          selectedTicket.chuyen_xe?.gio_khoi_hanh?.substring(
                            0,
                            5,
                          )
                        }}</span
                    ...
                    <div class="flex justify-between items-start">
                      ...
                      <div class="text-right">
                        ...
                        <p
                          ...
                          "
                          class="text-sm text-slate-900 mt-0.5"
                        >
                          {{
                            selectedTicket.chi_tiet_ves?.[0]?.tram_don?.dia_chi
                          }}
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      >
                      <span class="font-bold text-slate-900 text-base">{{
                        selectedTicket.chuyen_xe?.xe?.bien_so || "---"
                      }}</span>
                    </div>
                  </div>
                </section>
                ...
                <section>
                  <h4 class="text-xl font-bold text-slate-900 mb-6">
                    Thông tin nhà xe
                  </h4>
                  <div class="space-y-4">
                    <div class="flex justify-between items-center">
                      ...
                      <span class="font-bold text-blue-600 text-base">{{
                        selectedTicket.chuyen_xe?.tuyen_duong?.nha_xe
                          ?.so_dien_thoai || "---"
                      }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      ...
                      >
                      <span class="font-bold text-blue-600 text-base">{{
                        selectedTicket.chuyen_xe?.tai_xe?.so_dien_thoai || "---"
                      }}</span>
                    </div>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div class="space-y-4">
                    <div class="flex justify-between items-center">
                      ...
                      >
                      <span class="font-bold text-slate-900 text-base">{{
                        selectedTicket.chuyen_xe?.tuyen_duong?.ten_tuyen_duong
                      }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-slate-500 font-semibold text-base"
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      >
                      <span class="font-bold text-slate-900 text-base">{{
                        selectedTicket.chuyen_xe?.xe?.loai_xe?.ten_loai_xe ||
                        "---"
                      }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      ...
                      >
                      <span class="font-bold text-slate-900 text-base"
                        >{{ selectedTicket.chi_tiet_ves?.length || 1 }} vé</span
                      >
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-slate-500 font-semibold text-base"
                      ...
                      <span class="font-bold text-slate-900 text-base">
                        {{
                          selectedTicket.phuong_thuc_thanh_toan === "tien_mat"
                            ? "Tiền mặt"
                            : "Chuyển khoản (VietQR)"
                        }}
                      </span>
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      <div class="flex items-center gap-1">
                        <span class="font-bold text-slate-900 text-base">{{
                          selectedTicket.vi_tri_ghe ||
                          (selectedTicket.chi_tiet_ves
                            ? selectedTicket.chi_tiet_ves
                                .map((ct) => ct.ghe?.ma_ghe)
                                .join(", ")
                            : "---")
                        }}</span>
                        <span
                          class="material-symbols-outlined text-sm text-slate-900"
                          >expand_less</span
                        >
                      </div>
                    </div>
                    <div
                      class="flex justify-between items-center pt-6 border-t border-slate-100"
                    >
                      <span
                        class="text-slate-900 font-bold uppercase tracking-widest text-sm"
                        >Tổng tiền</span
                      >
                ...
                <section>
                  ...
                  <div class="space-y-4">
                    <div class="flex justify-between items-center">
                      ...
                      >
                      <span class="font-bold text-slate-900 text-base">{{
                        selectedTicket.khach_hang?.ho_va_ten
                      }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-slate-500 font-semibold text-base"
                      ...
                      >
                      <span class="font-bold text-blue-600 text-base">{{
                        selectedTicket.khach_hang?.so_dien_thoai
                      }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                      <span class="text-slate-500 font-semibold text-base"
                      ...
                      >
                      <span class="font-bold text-slate-900 text-base">{{
                        selectedTicket.khach_hang?.email
                      }}</span>
                    </div>
                  </div>
                </section>
              ...
              <div class="trip-rating-detail-body">
                <div class="trip-rating-detail-trip">
                  ...
                  <div class="trip-rating-detail-trip-route">
                    {{ getRatingRouteLine(selectedMyRating) }}
                  </div>
                ...
                <div class="trip-rating-detail-top">
                  <span class="trip-rating-detail-name">{{
                    getRatingProfileName(selectedMyRating)
                  }}</span>
                    ...
                    <span
                      ...
                      :class="{
                        'trip-rating-detail-star--on':
                          star <= clampRatingScore(selectedMyRating?.diem_so),
                      }"
                      aria-hidden="true"
                      >★</span
                    >
                    <span class="trip-rating-detail-score"
                      >{{ clampRatingScore(selectedMyRating?.diem_so) }}/5</span
                    >
                  </div>
                </div>
                    ...
                    <div class="trip-rating-detail-metric-stars">
                      <span
                        ...
                        :class="{
                          'trip-rating-detail-star--on':
                            star <=
                            subRatingStars(selectedMyRating?.diem_dich_vu),
                        }"
                        aria-hidden="true"
                        >★</span
                      >
                      <strong>{{
                        subRatingLabel(selectedMyRating?.diem_dich_vu)
                      }}</strong>
                    </div>
                  </div>
                  <div class="trip-rating-detail-metric">
                    ...
                    <div class="trip-rating-detail-metric-stars">
                      <span
                        ...
                        :class="{
                          'trip-rating-detail-star--on':
                            star <=
                            subRatingStars(selectedMyRating?.diem_an_toan),
                        }"
                        aria-hidden="true"
                        >★</span
                      >
                      <strong>{{
                        subRatingLabel(selectedMyRating?.diem_an_toan)
                      }}</strong>
                    </div>
                  </div>
                  <div class="trip-rating-detail-metric">
                    ...
                    <div class="trip-rating-detail-metric-stars">
                      <span
                        ...
                        :class="{
                          'trip-rating-detail-star--on':
                            star <=
                            subRatingStars(selectedMyRating?.diem_sach_se),
                        }"
                        aria-hidden="true"
                        >★</span
                      >
                      <strong>{{
                        subRatingLabel(selectedMyRating?.diem_sach_se)
                      }}</strong>
                    </div>
                  </div>
                  <div class="trip-rating-detail-metric">
                    ...
                    <div class="trip-rating-detail-metric-stars">
                      <span
                        ...
                        :class="{
                          'trip-rating-detail-star--on':
                            star <=
                            subRatingStars(selectedMyRating?.diem_thai_do),
                        }"
                        aria-hidden="true"
                        >★</span
                      >
                      <strong>{{
                        subRatingLabel(selectedMyRating?.diem_thai_do)
                      }}</strong>
                    </div>
                  </div>
                </div>
                ...
                <div
                  class="trip-rating-detail-trip-meta trip-rating-detail-sent"
                >
                  Gửi lúc:
                  {{ formatRatingDateTime(selectedMyRating.created_at) }}
                </div>
                <p class="trip-rating-detail-note">
                  {{ selectedMyRating?.noi_dung || "Không có nhận xét." }}
                </p>
              </div>
            </div>
          </div>
