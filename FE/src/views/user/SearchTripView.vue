...
/** Đồng bộ lọc giờ từ query (deep link từ chat / chia sẻ URL). */
const applyRouteTimeFilters = (q) => {
  const tu = q.gio_khoi_hanh_tu ? String(q.gio_khoi_hanh_tu) : "";
  const den = q.gio_khoi_hanh_den ? String(q.gio_khoi_hanh_den) : "";
  if (!tu && !den) {
    filters.value.gio_khoi_hanh_tu = "";
    filters.value.gio_khoi_hanh_den = "";
    selectedPredefinedTime.value = null;
  ...
  filters.value.gio_khoi_hanh_tu = tu;
  filters.value.gio_khoi_hanh_den = den;
  const match = predefinedTimeFilters.find((x) => x.tu === tu && x.den === den);
  selectedPredefinedTime.value = match ? match.value : null;
};
...
watch(
  () => route.query,
  (newQuery) => {
    applyRouteTimeFilters(newQuery || {});
    searchParams.value = {
...
onMounted(() => {
  fetchProvinces();
  applyRouteTimeFilters(route.query || {});
  performSearch();
