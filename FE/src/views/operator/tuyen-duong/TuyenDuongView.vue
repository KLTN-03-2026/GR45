...
const addStation = () => {
  formData.tram_dungs.push(getEmptyStation('ca_hai', formData.tram_dungs.length + 1))
}

const removeStation = (index) => {
  formData.tram_dungs.splice(index, 1)
  // Cập nhật lại thứ tự sau khi xóa
  formData.tram_dungs.forEach((s, i) => { s.thu_tu = i + 1 })
}
...
const handleSearch = () => { fetchRoutes(1) }
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      showToast('Cập nhật tuyến đường thành công! Đang chờ Admin duyệt lại.', 'success')
    } else {
      await operatorApi.createRoute(formData)
      showToast('Thêm tuyến đường thành công! Đang chờ Admin duyệt.', 'success')
    }
  ...
  <div class="operator-page">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Tuyến đường mới cần Admin duyệt trước khi hoạt động</p>
      </div>
    ...
    <div class="filter-card">
      <div class="filter-row">
        ...
        <div class="filter-status">
          ...
          <select v-model="filterStatus" @change="handleSearch" class="custom-select">
        ...
        <BaseButton @click="handleResetFilter" variant="outline">Đặt lại</BaseButton>
      </div>
    </div>
    ...
    <div class="table-card">
      ...
      >
        ...
        <template #cell(gio_khoi_hanh)="{ item }">
          <span class="time-range">{{ item.gio_khoi_hanh }} – {{ item.gio_ket_thuc }}</span>
        </template>
        ...
        <template #cell(so_tram)="{ item }">
          <span class="station-count">{{ (item.tram_dungs || []).length }} trạm</span>
        </template>
        ...
        <template #cell(actions)="{ item }">
          <div class="action-buttons">
            <BaseButton size="sm" variant="outline" @click="openDetailModal(item)">
              Chi tiết
            </BaseButton>
            <BaseButton size="sm" variant="primary" @click="openEditModal(item)">
              Sửa
            </BaseButton>
          </div>
        </template>
      ...
      <div class="pagination-container">
        <div class="page-info-left">
          <span>Hiển thị:</span>
          <select v-model="pagination.perPage" @change="fetchRoutes(1)" class="custom-select per-page-select">
          ...
          <span v-if="pagination.total > 0" class="total-label">(Tổng: {{ pagination.total }} tuyến)</span>
        </div>
    ...
    >
      ...
      <div class="info-banner">
        ...
        <span v-if="isEditMode">
          Sau khi cập nhật, tuyến đường sẽ chuyển sang <strong>Chờ Admin duyệt lại</strong>.
        </span>
        <span v-else>
          Tuyến đường mới sẽ có trạng thái <strong>Chờ Admin duyệt</strong> trước khi hoạt động.
        </span>
      </div>
      ...
      <form @submit.prevent="submitForm" class="route-form">
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input type="number" v-model="formData.gio_du_kien" class="custom-input" min="0" step="0.5" />
          </div>

          <div class="form-group">
            <label class="base-input-label">ID Xe (tùy chọn)</label>
            <input type="number" v-model="formData.xe" class="custom-input" min="1" placeholder="Nhập ID xe..." />
          </div>

          <div class="form-group full-width">
            <label class="base-input-label">Ghi Chú / Mô Tả</label>
            <textarea v-model="formData.mo_ta" class="custom-input custom-textarea" placeholder="Mô tả thêm về tuyến đường..."></textarea>
          </div>
        </div>
        ...
        <div class="days-grid">
          <label
            ...
            :key="day.value"
            class="day-checkbox"
            :class="{ 'day-active': formData.cac_ngay_trong_tuan.includes(day.value) }"
          >
            <input
              type="checkbox"
        ...
        <div class="stations-header mt-4">
          ...
          <BaseButton size="sm" variant="outline" type="button" @click="addStation">
            + Thêm Trạm
          </BaseButton>
        </div>
          ...
          <div v-for="(station, index) in formData.tram_dungs" :key="index" class="station-item">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input type="number" step="0.000001" v-model="station.toa_do_x" class="custom-input" placeholder="VD: 106.6297" />
              </div>

              <div class="form-group">
                <label class="base-input-label">Vĩ Độ (Latitude)</label>
                <input type="number" step="0.000001" v-model="station.toa_do_y" class="custom-input" placeholder="VD: 10.8231" />
              </div>
    ...
    <!-- ===== MODAL CHI TIẾT ===== -->
    <BaseModal v-model="isDetailModal" title="Chi Tiết Tuyến Đường" maxWidth="700px">
      <div v-if="detailLoading" class="detail-loading">Đang tải...</div>

      <div v-else-if="detailData" class="detail-body">

        <!-- Trạng thái nổi bật -->
        <div class="detail-status-banner" :class="getStatusLabel(detailData.tinh_trang).class">
          <span>{{ getStatusLabel(detailData.tinh_trang).text }}</span>
          <span v-if="detailData.tinh_trang === 'cho_duyet'" class="status-note">— Đang chờ Admin duyệt</span>
          <span v-if="detailData.tinh_trang === 'khong_hoat_dong'" class="status-note">— Đã bị từ chối hoặc chưa hoạt động</span>
        </div>
        ...
        <div class="detail-grid">
          <div class="detail-item full-width">
            ...
            <span class="detail-value route-name">{{ detailData.ten_tuyen_duong }}</span>
          </div>
          ...
          <div class="detail-item">
            ...
            <span class="detail-value price-value">{{ formatCurrency(detailData.gia_ve_co_ban) }}</span>
          </div>
          <div class="detail-item">
          ...
          <div class="detail-item">
            ...
            <span class="detail-value">{{ detailData.gio_du_kien || '—' }} giờ</span>
          </div>
          <div class="detail-item full-width">
            <span class="detail-label">Lịch chạy</span>
            <span class="detail-value">{{ formatDays(detailData.cac_ngay_trong_tuan) }}</span>
          </div>
        ...
        <h4 class="detail-section-title">
          Trạm Dừng
          <span class="station-count-badge">{{ (detailData.tram_dungs || []).length }} trạm</span>
        </h4>
          ...
          >
            ...
            <div class="timeline-content">
              <div class="timeline-header">
                ...
                <span class="loai-tram-badge">{{ loaiTramLabel(tram.loai_tram) }}</span>
              </div>
      ...
      <template #footer>
        ...
        <BaseButton variant="primary" @click="openEditModal(detailData); isDetailModal = false">
          Chỉnh Sửa
        </BaseButton>
      </template>
    </BaseModal>
...
.search-box > :first-child { flex: 1; margin-bottom: 0; }
.filter-status { display: flex; flex-direction: column; gap: 4px; }
...
.point-start { font-weight: 600; color: #16a34a; }
.point-end { font-weight: 600; color: #0d4f35; }
.route-arrow { color: #94a3b8; font-weight: bold; font-size: 16px; }
.km-value { font-weight: 600; color: #475569; }
.time-range { font-size: 13px; color: #475569; font-weight: 500; }
.price-value { font-weight: 700; color: #16a34a; }
.station-count { font-size: 13px; color: #64748b; }
...
.per-page-select { width: 70px !important; display: inline-block; }
...
.base-input-label { font-size: 13px; font-weight: 500; color: #374151; }
...
.day-checkbox:hover { border-color: #16a34a; background: #f0fdf4; }
.day-checkbox.day-active {
  border-color: #16a34a;
  background: #16a34a;
}
.day-checkbox.day-active .day-label { color: white; font-weight: 700; }
.day-label { font-size: 13px; font-weight: 600; color: #374151; }
...
.station-item:hover { border-color: #bbf7d0; }
.station-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
...
.station-order { display: flex; align-items: center; gap: 10px; }
.station-num {
  width: 28px;
  height: 28px;
...
.detail-status-banner.status-approved { background: #dcfce7; color: #16a34a; }
.detail-status-banner.status-pending { background: #fef9c3; color: #ca8a04; }
.detail-status-banner.status-rejected { background: #fee2e2; color: #dc2626; }
.status-note { font-weight: 400; font-size: 13px; opacity: 0.8; }
...
.route-name { font-size: 16px; font-weight: 800; color: #0d4f35; }
...
.loai-don .timeline-dot { background: #22c55e; }
.loai-tra .timeline-dot { background: #ef4444; }
.loai-ca_hai .timeline-dot { background: #3b82f6; }
...
.loai-don .loai-tram-badge { background: #dcfce7; color: #16a34a; }
.loai-tra .loai-tram-badge { background: #fee2e2; color: #dc2626; }
.loai-ca_hai .loai-tram-badge { background: #eff6ff; color: #2563eb; }

.timeline-addr { font-size: 12px; color: #64748b; margin: 2px 0; }
.timeline-coords { font-size: 11px; color: #94a3b8; margin: 0; }
