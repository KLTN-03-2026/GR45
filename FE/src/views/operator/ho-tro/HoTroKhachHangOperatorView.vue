...
const { subscribe, unsubscribeAll, isSubscribed } =
  useLiveSupportChannel("nha_xe_panel");
...
async function fetchSessions(page = 1, append = false) {
  ...
  try {
    const envelope = await axiosClient.get("/v1/nha-xe/ho-tro/khach-hang/sessions", {
      params: {
...
const loadMoreMessages = async () => {
  ...
  try {
    const envelope = await axiosClient.get(
      `/v1/nha-xe/ho-tro/khach-hang/sessions/${currentSessionId.value}?page=${currentMessagePage.value + 1}`,
    );
    const payload = envelope?.data ?? {};
...
const sendMessage = async () => {
  ...
  try {
    const envelope = await axiosClient.post(
      `/v1/nha-xe/ho-tro/khach-hang/sessions/${currentSessionId.value}/reply`,
      {
        content,
      },
    );
...
const resolveSession = async () => {
  ...
  try {
    const envelope = await axiosClient.post(
      `/v1/nha-xe/ho-tro/khach-hang/sessions/${currentSessionId.value}/resolve`,
    );
...
<template>
  <div class="chat-support-container d-flex flex-column gap-3">
    <SupportChatStatsChart
      class="mx-chat-stats"
      api-path="/v1/nha-xe/ho-tro/khach-hang/stats-daily"
      scope-hint="Chỉ nhà xe đăng nhập — khách chat tới nhà xe."
    />
        ...
        >
          <div class="d-flex align-items-center justify-content-between gap-2">
            <h5 class="m-0 fw-bold text-primary d-flex align-items-center mb-0">
              <i class="bi bi-people me-2"></i>Hỗ trợ khách (live)
            </h5>
          ...
          <p
            v-if="operatorScopeLabel"
            class="small text-muted mb-0 lh-sm"
          >
            Chỉ khách chat tới nhà xe đăng nhập: {{ operatorScopeLabel }}
          </p>
        </div>
