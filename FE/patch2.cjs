...
const prefix = lines.slice(0, 198); // 0 to 197
const suffix = lines.slice(336); // 336 to end

const newFunction = `const routeToStop = async (tram) => {
  activeTargetStop.value = tram;
  // reset text distance
  tram.routingDistance = null;
  tram.routingDuration = null;

  // OpenMap Direction API yêu cầu định dạng lat,lng
  ...
  try {
    ...
    if (data.routes && data.routes.length > 0) {
      ...
      // Lấy khoảng cách & thời gian nếu API OpenMap trả về \`legs\`
