import { anyTrue, textOrEmpty, valueOr } from "../value.js";
...
export function shouldRouteStandardThroughPlanner(text, options = {}) {
  ...
  if (patternMatches(valueOr(options.policyPattern, POLICY_RE), n)) return false;
  if (patternMatches(valueOr(options.moneyRiskPattern, MONEY_RISK_RE), n)) return false;
...
export function shouldPreferPdfAfterBarrenTools(text, options = {}) {
  ...
  return ["policy_or_document", "long_form"].includes(intent);
}

export function classifyIntentText(text, options = {}) {
  const rawText = textOrEmpty(text);
  ...
  const isStrongKnowledgeQuery = patternMatches(
    valueOr(options.strongKnowledgeQueryPattern, STRONG_KNOWLEDGE_QUERY_RE),
    normalized,
  );
  const isOperational =
  ...
  const isPolicyQuestion = patternMatches(
    valueOr(options.policyPattern, POLICY_RE),
    normalized,
  );
  const isMoneyRisk =
    !isStrongKnowledgeQuery &&
    patternMatches(valueOr(options.moneyRiskPattern, MONEY_RISK_RE), normalized);
  ...
  const isKnowledgeQuery =
    anyTrue(
      ...
      !isPolicyQuestion &&
      patternMatches(
        valueOr(options.knowledgeQueryPattern, KNOWLEDGE_QUERY_RE),
        normalized,
      ),
    );

  const docOrKnowledgeIntent = anyTrue(isPolicyQuestion, isKnowledgeQuery);
  ...
  return {
    intent,
    isOperational,
    isPolicyQuestion: anyTrue(isPolicyQuestion, isKnowledgeQuery),
    isKnowledgeQuery,
    isMoneyRisk,
    normalized,
  };
