...
export function getSlotDescription(slotName) {
  if (COMMON_SLOTS[slotName]) return COMMON_SLOTS[slotName];
  if (EXTENDED_SLOTS[slotName]) return EXTENDED_SLOTS[slotName];
  return slotName;
}

export function expandSlotExpression(slotExpression) {
  return String(slotExpression == null ? "" : slotExpression)
    .split("|")
    .map((slot) => slot.trim())
    .filter(Boolean);
}
...
export function getSlotsForToolSpec(spec = {}) {
  const conditionalSlots = Object.values(
    spec.conditionalRequiredSlots ? spec.conditionalRequiredSlots : {},
  ).flat();
...
export function buildCompactToolDescription(toolName, spec = {}) {
  ...
  return [
    spec.shortDescription ? spec.shortDescription : inferToolPurpose(toolName),
